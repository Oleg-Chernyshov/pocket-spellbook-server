...
describe('AuthController (e2e)', () => {
  ...
  describe('/auth/refresh (POST)', () => {
    it('should refresh tokens with valid refresh token', async () => {
      ...
      // Вход для получения refresh-токена
    ...
    it('should not accept access token as refresh token', async () => {
      ...
      // Попытка использовать access-токен вместо refresh-токена
      return request(app.getHttpServer())
        .post('/auth/refresh')
  ...
  describe('/auth/logout (POST)', () => {
    ...
    it('should invalidate refresh token after logout', async () => {
      ...
      // Попытка использовать refresh-токен после выхода
      return request(app.getHttpServer())
        .post('/auth/refresh')
        .set('Authorization', `Bearer ${refresh_token}`)
        .expect(401);
  ...
  describe('Refresh Token Flow (Integration)', () => {
    it('should handle complete auth flow: register -> login -> refresh -> logout', async () => {
      ...
      // 4. Старый refresh-токен не должен работать после ротации
      await request(app.getHttpServer())
        .post('/auth/refresh')
        .set('Authorization', `Bearer ${firstRefreshToken}`)
        .expect(401);

      // 5. Выход с новым access-токеном
      await request(app.getHttpServer())
        .post('/auth/logout')
        .set('Authorization', `Bearer ${newAccessToken}`)
        .expect(200);

      // 6. Новый refresh-токен не должен работать после выхода
      return request(app.getHttpServer())
        .post('/auth/refresh')
